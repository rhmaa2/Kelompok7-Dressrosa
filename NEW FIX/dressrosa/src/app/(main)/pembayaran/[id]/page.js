...
import { getPeminjamanById, getPembayaranList, buatPembayaran } from "@/lib/store";
...
export default function PembayaranPage() {
  ...
  const [kategori, setKategori] = useState("Transfer");
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!mounted) return;
        setP(peminjaman || false);

        if (peminjaman) {
          const pembayaranList = await getPembayaranList();
          const found = pembayaranList.some(
            (x) => String(x.peminjaman_id) === String(id)
          );
          if (mounted) setSudahBayar(found);
        }
      } catch {
      ...
      <div className="mx-auto max-w-lg px-4 py-16 text-center">
        <div className="rounded-2xl border bg-white p-8 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Pengajuan #{p.id} sudah dibayar dan sedang diproses/diverifikasi admin.
          </p>
          <Link
            href={`/status/${p.id}`}
  ...
  async function konfirmasiBayar() {
    ...
    setError("");
    try {
      await buatPembayaran({
        peminjamanId: p.id,
        jumlahBayar: p.totalBayar,
        metodeBayar: kategori === "Transfer" ? "virtual_account" : "qris",
      });
